...
function addMetadata(packname, author) {
				if (!packname) packname = 'bot-wa-with-termux'; if (!author) author = ' AgatZ';
...
async function starts() {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const botNumber = agat.user.jid
			const ownerNumber = [`${083121562064}@s.whatsapp.net`] // owner number ubah aja 
			const ownerInfo = `${083121562064}`
			...
			const AgatZ = ["0@s.whatsapp.net"]
			const AgatZ2 = " ~ 𝐂𝐫𝐞𝐚𝐭𝐞𝐝 𝐁𝐲 AgatZ"
			const isUrl = (url) => {
...
return new Promise(resolve => setTimeout(resolve, ms));
		...
		break
			...
			case 'brainly':
					...
					costum(teks, text, AgatZ, `Ciee Cari Jawaban Yaa😂\nFollow IG: @AgatZShop`)
					console.log(res)
					})
					await limitAdd(sender)
				break 
		...
		case 'verify':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen ANKOCHANBOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				user.push(sender)
				costum(help(prefix, instagram, yt, name, pushname2, user, limitt, uptime, jam, tanggal(), groupName, premi, Simihh, Welcomee, ModeAnime, Nsfww, BadWordd, AntiLinkk, AntiVirtexx), text, AgatZ, AgatZ2)
			break 

// fitur simple
		case 'listsurah':
				await costum(listsurah(), text, AgatZ, AgatZ2)
			break 
			case 'menumedia':
				user.push(sender)
				await costum(mediaa(prefix, pushname2, groupName, name), text, AgatZ, AgatZ2)
			break 
			case 'menucreator':
				user.push(sender)
				await costum(creator(prefix, pushname2, groupName, user, name), text, AgatZ, AgatZ2)
			break 
			case 'menufun':
				user.push(sender)
				await costum(fun(prefix, pushname2, groupName, user, name), text, AgatZ, AgatZ2)
			break 
			case 'menuspam':
				user.push(sender)
				await costum(spam(prefix, pushname2, groupName, user, name), text, AgatZ, AgatZ2)
			break 
			case 'menuinformasi':
				user.push(sender)
				await costum(informasi(prefix, pushname2, groupName, user, name), text, AgatZ, AgatZ2)
			break 
			case 'menuprimbon':
				user.push(sender)
				await costum(primbon(prefix, pushname2, groupName, user, name), text, AgatZ, AgatZ2)
			break 
			case 'menuencrypt':
				user.push(sender)
				await costum(encrypt(prefix, pushname2, groupName, user, name), text, AgatZ, AgatZ2)
			break 
			case 'menugroup':
				user.push(sender)
				uptime = process.uptime()
				await costum(groupp(prefix, pushname2, groupName, user, name, uptime, jam, tanggal(), premi, Simihh, Welcomee, ModeAnime, Nsfww, BadWordd, AntiLinkk, AntiVirtexx), text, AgatZ, AgatZ2)
			break 
			case 'menupremium':
				user.push(sender)
				premium.push(sender)
				await costum(menupremium(prefix, pushname2, groupName, user, name, premi), text, AgatZ, AgatZ2)
			break 
			case 'menuothers':
				user.push(sender)
				await costum(others(prefix, pushname2, groupName, user, name), text, AgatZ, AgatZ2)
			break 
			case 'menuowner':
				user.push(sender)
				await costum(ownerrr(prefix, pushname2, groupName, user, name), text, AgatZ, AgatZ2)
			break 
			case 'menuscrapper':
				user.push(sender)
				await costum(scrapper(prefix, pushname2, groupName, user, name), text, AgatZ, AgatZ2)
			break 
// end fitur simple

			case 'infobot':
				await costum(bottt(prefix), text, AgatZ, botinfo)
					break
				...
				case 'sticker':
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('AgatZ', 'Jangan Lupa Donasi')} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(mess.error.stick)
									await costum(fs.readFileSync(ran), sticker, AgatZ, ` ~ Nihh Udah Jadi Stikernya`)
									fs.unlinkSync(media)	
									fs.unlinkSync(ran)	
								})
							})
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('AgatZ', 'Jangan Lupa Donasi')} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(mess.error.stick)
									await costum(fs.readFileSync(ran), sticker, AgatZ, `~ Nih Dah Jadi Gif Stikernya`)
									fs.unlinkSync(media)
									fs.unlinkSync(ran)
								})
							})
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('AgatZ', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
					...
					fs.unlinkSync(ranp)
					if (err) return reply(mess.error.stick)
					exec(`webpmux -set exif ${addMetadata('AgatZ', 'Jangan Lupa Donasi')} ${rano} -o ${rano}`, async (error) => {
					if (error) return reply(mess.error.stick)
			...
			case 'tagme':
				...
				if (!isUser) return reply(mess.only.userB)
				await costum(`@${sender.split('@')[0]}`, text, AgatZ, `@${sender.split('@')[0]}`)
				break
			case 'lirik':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
//creator
			...
			const	textnya = teks.split("/")[2];
					...
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, AgatZ, mess.success)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (args.length < 1) {
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(anu), image, AgatZ, AgatZ2)
					} else {
			...
			case 'ttp':
				...
				reply(mess.wait)
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, AgatZ, `~ Emoji Maker`)
					} else {
					reply(mess.error.bug)
					}
					 await limitAdd(sender)
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, AgatZ, ` ~ Game Logo Text Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, AgatZ, ` ~ Facebook Gold Button Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, AgatZ, ` ~ Black Pink Text Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, AgatZ, ` ~ Party Text Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, AgatZ, ` ~ Style Logo Text Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, AgatZ, ` ~ Wet Glass Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				case 'croman':
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, AgatZ, ` ~ Romance Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				case 'clove':
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, AgatZ, ` ~ Love Message Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender)
				...
				reply(mess.wait)
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, AgatZ, ` ~ Harta Tahta ${args[0]}`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				try {
					...
					buffer = await getBuffer(anu.result)
					costum(buffer, image, AgatZ, ` ~ Quotes Maker`)
					} catch {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				case 'cphlogo':
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, AgatZ, ` ~ Pornhub Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				case 'cwolf':
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, AgatZ, ` ~ Wolf Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(anu), image, AgatZ, ` ~ Glitch Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(anu), image, AgatZ, ` ~ Ballom Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
...
//akhir kreator
		    ...
		    try {
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 ANKO BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 ANKO BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    try {
			    if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						costum(buffer, image, AgatZ, AgatZ2)
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break 
          ...
          case 'snack':
			...
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (anu.error) return reply(anu.error)
                ...
                bufferddd = await getBuffer('https://raw.githubusercontent.com/AgatZCode/bot-wa-with-termux/main/src/glitchtext.png')
                 reply(mess.wait)
                buff = await getBuffer(anu.result)
		...
		break 
			...
			case 'randombts':
				...
				frhan.sendMessage(from, buff, image, {quoted: mek, caption: 'BTS.....'})
					...
					await limitAdd(sender)
			case 'antivirtexx':
			await costum(antivirtexx(), text, AgatZ, `Buset Dahh Etekel🔥`)
			break 
			case 'addstatus':
					if (!isOwner) return reply(mess.only.ownerB)
		...
		break 
				default:
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ANKOCHANBOT]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
			}
		} catch (e) {
